...
const Footer=()=>
{
    return <div className="pt-20 pb-5 flex gap-5 justify-around font-['poppins']" style={{backgroundColor:'#2d2d2d'}}>
 ...
 <div className="flex gap-3 items-center text-bright-sun-400">
      ...
      <div className="text-sm text-mine-shaft-300 ">
        Job portal with user profiles,skill updates, certifications, work experience and 
        admin postings.
      </div>
            ...
            footerLinks.map((item,index)=> <div key={index}>
                <div className="text-lg font-semibold mb-4 text-bright-sun-400">{item.title}</div>
                {
                    ...
                    text-sm hover:text-bright-sun-400 cursor-pointer mb-1 hover:translate-x-2
                     transition duration-300 ease-in-out">{link}</div>)
                }
                </div>)
        }
    </div>
